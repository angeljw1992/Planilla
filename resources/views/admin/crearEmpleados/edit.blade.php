...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.crearEmpleado.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.crear-empleados.update", [$crearEmpleado->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.crearEmpleado.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $crearEmpleado->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('codigo_empleado') ? 'is-invalid' : '' }}" type="text" name="codigo_empleado" id="codigo_empleado" value="{{ old('codigo_empleado', $crearEmpleado->codigo_empleado) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('rol') ? 'is-invalid' : '' }}" name="rol" id="rol" required>
                    ...
                    @foreach(App\Models\CrearEmpleado::ROL_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('rol', $crearEmpleado->rol) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('rol'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('restaurante') ? 'is-invalid' : '' }}" name="restaurante" id="restaurante" required>
                    ...
                    @foreach(App\Models\CrearEmpleado::RESTAURANTE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('restaurante', $crearEmpleado->restaurante) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
